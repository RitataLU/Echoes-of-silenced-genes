"""
GNN v2 training — LOCAL (CPU/Mac) variant.

Identical to train_v2.py except:
- Uses config_local.py (smaller model, fewer epochs, CPU-friendly)
- Checkpoints: v2_local_fold{k}_best.pt  (no overwrite of GPU checkpoints)
- OOF output:  oof_gnn_v2_local.csv

Usage
-----
    python src/train_v2_local.py
    python src/train_v2_local.py --k 16   # even fewer programs
"""
...
def train_fold(fold, train_indices, val_indices,
    ...
    # LOCAL checkpoint name — does not overwrite GPU checkpoints
    ckpt_path  = os.path.join(config.CHECKPOINTS_DIR, f"v2_local_fold{fold}_best.pt")
...
def plot_curves(all_history, suffix="v2_local"):
...
def main():
    ...
    print(f"Device: {device}  (local config: hidden={config.GNN_HIDDEN_DIM}, "
          f"layers={config.GNN_NUM_LAYERS}, epochs={config.MAX_EPOCHS})")
    ...
    try:
        ...
        oof_df.to_csv(os.path.join(config.SUBMISSIONS_DIR, "oof_gnn_v2_local.csv"), index=False)

    finally:
        if all_history:
            plot_curves(all_history, suffix="v2_local")
